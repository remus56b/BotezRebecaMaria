...
function serveMedia(request, response, url) {
  ...
  if (!isInsideDirectory(filesRoot, file) || !existsSync(file)) return sendNotFound(response);
...
function serveAsset(response, pathname) {
  ...
  if (!isInsideDirectory(join(publicRoot, 'assets'), file) || !existsSync(file)) return sendNotFound(response);
